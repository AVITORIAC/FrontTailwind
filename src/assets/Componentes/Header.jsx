...
function Header({ gestor, instrutor, home, aprendiz, admin}) {
  const cores=`
  ${gestor ? 'bg-gestor' : ''}
  ...
  if (home) {
    return(
      ...
      <p className={'text-3xl font-bold absolute inset-x-0 top-4 left-32 h-16 text-white'}>Meinung</p>
      <FiLogIn color='white' className={' size-9 absolute top-4 right-4 h-10 w-10'}></FiLogIn>
     </div>
    )
  }
  return (
